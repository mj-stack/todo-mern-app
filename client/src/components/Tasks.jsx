...
const Tasks = ({ displayTask, setDisplayTask }) => {
  ...
  return (
    <section className="ml-auto mr-auto w-[60%] grid grid-cols-1 justify-center gap-3 h-[130px] overflow-scroll no-scrollbar">
      {displayTask.map((task, index) => (
        ...
        >
          <div>{task.task}</div> {/* Display the task name */}
